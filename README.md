...
生成的预分类报告包含：
- 各部门Bug数量总览
- 疑似非Bug清单（type=performance，建议人工确认排除）
- 外部Bug责任界定（含Bug现象、影响范围、可能原因、复盘建议）
- 内部Bug责任界定（含Bug现象、可能原因、预测争议点）
- 低质量任务分析（多维度识别，区分「提测质量差」和「需求质量差」）

报告自动保存到 `~/.bsg-zentao/报告/Bug界定/`。

### 版本复盘（复盘会正式文档）

在复盘会前或复盘会上生成，输出完整的复盘报告文档。
...
报告包含外部Bug复盘（趋势+深度分析）、内部Bug复盘、版本需求趋势、延期分析等完整内容。

报告自动保存到 `~/.bsg-zentao/报告/版本复盘/`。

> ⚠️ **Bug 界定 ≠ 版本复盘**
> 两者是完全不同的东西，用不同的工具生成：
> - **Bug界定**：复盘会前的预分类准备材料，帮你判断哪些Bug值得复盘
> - **版本复盘**：复盘会上展示的正式报告，有历史趋势和管理结论
>
...
| Bug界定           | ✅ 可用   | "帮我出Bug界定报告" / "出预分类"                 |
| 版本复盘            | ✅ 可用   | "帮我出版本复盘" / "生成复盘报告"                  |
...
两者是完全不同的东西：Bug界定是复盘会前的预分类准备材料（周三出），帮你判断哪些Bug值得复盘、归属是什么；版本复盘是复盘会上展示的正式报告（含趋势、深度分析、管理结论）。直接说「复盘」时，Claude Code 会先问你要哪个。
...
| 版本   | 日期         | 说明                                                                                  |
| ---- | ---------- | ------------------------------------------------------------------------------------- |
| v1.4 | 2026-04-14 | Bug界定分析引擎优化：新增Bug现象推断、争议预测（数据信号驱动，不依赖人工标记）、低质量任务多维根因分类；明确 Bug界定 vs 版本复盘消歧规则 |
| v1.3 | 2026-04-14 | Bug界定场景上线，新增 `calc_bug_review.py`；修正 type=performance 处理逻辑、争议关键词扩展、禁止 deptName 回落 |

...
def calc_next_workload(
    ...
    版本交付判断（下一版本）：
      1. deadline 有值时，优先用 deadline
      2. deadline 为空时，再回退用 main_deadline
      3. main_deadline 为空时，再回退用 delivery_date
      4. 三者都为空时，不计入“版本交付”列

    原因：
      - deadline 更接近任务实际承诺时间
      - main_deadline 更接近禅道页面展示的主任务截止时间
      - delivery_date 更像 PM 计划日期，可作为最后兜底参考
      - 三者都为空时，默认算进版本交付会把统计明显放大
    ...
    for pool in pools:
        ...
        # 版本交付判断：deadline 优先，main_deadline 次之，delivery_date 最后兜底
        deadline = pool.get("deadline", "") or ""
        main_deadline = pool.get("main_deadline", "") or ""
        delivery_date = pool.get("delivery_date", "") or ""

        if deadline and deadline != _DATE_ZERO:
            in_v = deadline <= version_end
        elif main_deadline and main_deadline != _DATE_ZERO:
            in_v = main_deadline <= version_end
        elif delivery_date and delivery_date != _DATE_ZERO:
            in_v = delivery_date <= version_end
        else:
            in_v = False
